function Initialize()
	...
	dirMeasure = SKIN:GetMeasure('SubSkinDirectionCalc')
end

function Update()
	--Check Skin position
	if dirMeasure:GetValue() == 1 then
...
function DrawSubProcessShape(TotalRunning,CurrentDrawingIndex,CurrentActiveWindow)
	...
	for j = 0,TotalRunning do
		SKIN:Bang('!SetOption SubprocessTrackingShape Shape'..shapeCount..' "Ellipse '..(IconGap + IconSize/2 + (IconGap*2 + IconSize) * CurrentDrawingIndex - totalDotSpace/2 + j*DotGap)..','..dotPosition..',2.5 | StrokeWidth 0 | Extend '..DotTrait..'"')
