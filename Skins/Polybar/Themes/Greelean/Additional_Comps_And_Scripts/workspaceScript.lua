...
function Update()
	...
	if totalWorkspace ~= -1 then
		...
		SKIN:Bang('!SetOption WorkspaceShape Shape "Rectangle 0,#Tile_Space#,([Workspace1:W]*'..trueTotalWorkspace..'),(#Bar_Height#-2*#Tile_Space#) | Extend Trait"')
	end
	...
	if timing > 0 and timing < maxTime then
		...
		if currentWorkspace <= maximumWorkspace then 
			SKIN:Bang('[!ShowMeter WorkspaceCurrent]'
					..'[!SetOPtion WorkspaceCurrent Shape "Rectangle '..slideAnimation..',#Tile_Space#,(#Workspace_Number_Gap#*2),(#Bar_Height#-2*#Tile_Space#) | Extend Trait"][!UpdateMeter WorkspaceCurrent][!Redraw]')
		else
